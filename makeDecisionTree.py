...
def importance(attr, examples):
    entropy_before = entropy(examples["play"].tolist().count(
        0), examples["play"].tolist().count(1))
    ...
    for val in attr:
        ...
        for attr_val in attr_vals:
            ...
            ent = (len(exs["play"].tolist())/total_count) * \
                entropy(exs["play"].tolist().count(0),
                        exs["play"].tolist().count(1))
            entropy_list.append(ent)
        entropy_after[val] = sum(entropy_list)

    information_gain = {key: entropy_before -
                        entropy_after[key] for key in entropy_after}
...
def initiator(encodings=None, dataset=None):
    ...
    examples = pd.read_csv(f'./{dataset}', delimiter=",", header=None,
                           names=list(encodings_dict.keys()), dtype=np.int64)
    print(examples)
    print(encodings_dict)
    print(list(encodings_dict.keys())[:-1])
    pprint(decisionTreeLearning(examples=examples, attr=list(
        encodings_dict.keys())[:-1], parent_examples=examples))
